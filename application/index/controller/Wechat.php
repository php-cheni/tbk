...
class Wechat extends Controller
{
	...
	public function index()
	{
		...
		if (!empty($postStr)) {
			...
			if (empty($keyword)) {
				...
				$contentStr = $match ? $this->trans($match[0]) : json_decode($this->http_request("http://openapi.tuling123.com/openapi/api/v2", json_encode(['perception' => ['inputText' => ['text' => $keyword]], 'userInfo' => ['apiKey' => '5207baf8666d4b609a64b360a56b1593', 'userId' => substr(md5($fromUsername), 20, 8)]], JSON_UNESCAPED_UNICODE)))->results[0]->values->text;
			}
		}
	...
	public function trans($tpwd)
	{
		...
		preg_match('/[1-9]([0-9]{9,12})/', $data['data']['url'], $match);
		...
		// $award = isset($good->coupon_amount) ? floor(($good->zk_final_price - $good->coupon_amount) * $good->commission_rate / 100) : floor($good->zk_final_price * $good->commission_rate / 100);

		// if ($award < 125) {
		// 	$award = '';
		// 	$tpwd = isset($good->coupon_share_url) ? $this->createtpwd('https:' . $good->coupon_share_url) : $this->createtpwd('https:' . $good->url);
		// } else {
		// 	$tlj = $this->createtlj($match[0], $award);
		// 	if ($tlj) {
		// 		$award = ' 红包：' . $tlj['money'];
		// 		$tpwd = $tlj['tpwd'];
		// 	} else {
		...
		$award = isset($good->coupon_amount) ? floor(($good->zk_final_price - $good->coupon_amount) * $good->commission_rate / 200) / 100 : floor($good->zk_final_price * $good->commission_rate / 200) / 100;
		$coupon = isset($good->coupon_amount) ? "  优惠券：￥" . $good->coupon_amount : "";
		if ($award > 1 && config('coupon')) {
			$tpwd=$this->createtlj($match[0], $award);
			if ($tpwd) {
				$award = ' 下单红包：￥'.$award;
			}else{
				$award = '';
		...
		return $notice . "【" . $good->short_title . "】售价：￥" . $good->zk_final_price . $coupon . $award  . " \n复制到淘寶打开：" . $tpwd;
	}
	...
	private function createtlj($goodid, $money)
	{
		// $rule = [500 => 0.8, 1000 => 0.6, 2000 => 0.5, 5000 => 0.4];
		// foreach ($rule as $key => $value) {
		// 	if ($money <= $key) {
		// 		$percentage =  floor($value * $money) / 100;
		// 	} else {
		// 		break;
		// 	}
		// }
		...
		fwrite($file, date('Y-m-d H:i:s') . PHP_EOL . json_encode($resp, JSON_UNESCAPED_UNICODE) . PHP_EOL);
		fclose($file);
		return $resp['result']['success'] ? $this->createtpwd($resp['result']['model']['send_url']) : false;
	}
